...
export default function People() {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-2 px-4 h-12 border-b border-line"><h1 className="font-semibold text-[15px]">People</h1><span className="text-[11px] text-ink-3 tnum">{people.length}</span>
        <form className="ml-auto flex gap-1" onSubmit={async e => { e.preventDefault(); if (name.trim()) { await addPerson({ name: name.trim() }); setName(""); } }}>
          <input id="new-person" className="field h-7 w-[180px] text-[12px]" placeholder="Add person" value={name} onChange={e => setName(e.target.value)} /><button className="btn sm"><Plus size={13} /></button>
        </form>
      </header>
      <div className="flex-1 overflow-auto p-4">
        {!people.length ? <div className="text-ink-3 text-[12.5px] max-w-[52ch]">People appear here automatically when you write <b>from:Name</b> in quick add, or when tasks are captured from their messages. Each person gets a page showing what you owe them and what they owe you.</div> : (
          <div className="border border-line rounded-lg bg-panel overflow-hidden">
            <div className="grid grid-cols-[1fr_110px_110px_110px] px-3 h-8 items-center bg-panel-2 border-b border-line text-[11px] font-semibold text-ink-3 uppercase tracking-wide"><span>Name</span><span>I owe them</span><span>They owe me</span><span>Overdue</span></div>
            {people.map(p => {
