...
- If the email contains a numbered or bulleted list of things to do, output ONE TASK PER ITEM, not one task summarising the list. Deepak's "invoice changes" mail listed 18 separate jobs and became a single task, which could never honestly be ticked off. Each item gets its own title and quotes its own line. An item that is plainly information rather than work ("Invoice 28 of UC is shared") is not a task — skip it.
